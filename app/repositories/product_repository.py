...
class ProductRepository:
    ...
    def get_by_id(self, category_id: int) -> Optional[Product]:
        return self.db.query(Product).options(joinedload(Product.category)).filter_by(id=category_id).first()
    ...
    def create(self, product_date: ProductCreate) -> Product:
        db_product = Product(**product_date.model_dump())
    ...
    def get_multiple_by_ids(self, product_ids: List[int]) -> List[Product]:
        return (
            self.db.query(Product).options(joinedload(Product.category)).filter(Product.id.in_(product_ids)).all()
        )